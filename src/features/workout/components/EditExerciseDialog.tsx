...
import type { WorkoutExercise } from '../hooks/useWorkoutTracker'
...
export const EditExerciseDialog: React.FC<EditExerciseDialogProps> = ({
  ...
  const [sets, setSets] = useState(exercise?.sets || [{ reps: 0, weight: 0 }])
  const [useBodyweight, setUseBodyweight] = useState(false)

  useEffect(() => {
    if (exercise) {
      // Проверяем, есть ли подходы с весом 0 (собственный вес)
      const hasBodyweight = exercise.sets.every((set) => set.weight === 0)
      setUseBodyweight(hasBodyweight)
      setSets(exercise.sets)
    }
  }, [exercise])

  const handleAddSet = () => {
    setSets([...sets, { reps: 0, weight: useBodyweight ? 0 : 0 }])
  }
            ...
            <Switch
              ...
              onChange={(e) => {
                ...
                setSets(
                  sets.map((set) => ({
                    ...set,
                    weight: e.target.checked ? 0 : set.weight,
                  })),
                )
              }}
              sx={{
