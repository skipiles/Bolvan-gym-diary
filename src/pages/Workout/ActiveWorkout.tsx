...
import { Add, Delete, Check, PhotoCamera, Save } from '@mui/icons-material'
...
export const ActiveWorkout: React.FC<ActiveWorkoutProps> = ({
  ...
  const [endTime, setEndTime] = useState(draftData?.end_time || '')
  const [errorMessage, setErrorMessage] = useState('')
  const [showError, setShowError] = useState(false)
  const [isSaving, setIsSaving] = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)

  const { saveDraft, autoSave } = useWorkoutDraft()
  ...
  const handleManualSave = async () => {
    ...
    setErrorMessage('Черновик сохранён')
    setShowError(true)
  ...
  return (
    <Container maxWidth="sm" sx={{ py: 3 }}>
      ...
      >
        <Alert
          severity={errorMessage === 'Черновик сохранён' ? 'success' : 'error'}
          sx={{
            bgcolor:
              errorMessage === 'Черновик сохранён' ? '#34C759' : '#FF3B30',
            color: '#fff',
          }}
        >
          {errorMessage}
